...
def make_tray(**kwargs):
    quits = []
    fake_rumps = SimpleNamespace(App=FakeApp, MenuItem=FakeMenuItem,
...
def test_menu_is_built():
    ...
    assert titles == ["Dictation enabled", "Edit dictionary…", "Quit LocalFlow"]
...
def test_toggle_flips_state_and_notifies():
    events = []
    tray, _ = make_tray(on_toggle=events.append)
...
def test_quit_notifies_and_quits_rumps():
    events = []
    tray, quits = make_tray(on_quit=lambda: events.append("quit"))
...
def test_edit_dictionary_opens_file(monkeypatch, tmp_path):
    ...
    tray, _ = make_tray(dictionary_path=path)
